...
if is_debug_mode():
    ...
    LOCAL_EXCEL_EXPORT_FILE = BASE_DIR / "data_debug" / "kassensturz_data.xlsx"
    LOCAL_TEXT_EXPORT_FILE = BASE_DIR / "data_debug" / "kassensturz_data.txt"
else:
    LOCAL_DB_FILE = BASE_DIR / "data" / "kassensturz.db"
    BACKUP_DIR = BASE_DIR / "data" / "backups"
    LOCAL_EXCEL_EXPORT_FILE = BASE_DIR / "data" / "kassensturz_data.xlsx"
    LOCAL_TEXT_EXPORT_FILE = BASE_DIR / "data" / "kassensturz_data.txt"
...
#DB uses None while excel uses empty strings for empty fields
...
def export_entries_to_text(file_path: Path):
    entries = fetch_all_entries()
    ...
    for entry in entries:
        rows.append([
            excel_safe_value(shorten_id(entry.get("id"))),
            excel_safe_value(entry.get("date")),
            excel_safe_value(entry.get("timestamp")),
            excel_safe_value(entry.get("event_name")),
            excel_safe_value(entry.get("counted_by")),
    ...
    content = format_ascii_table(headers, rows)
    file_path.parent.mkdir(parents=True, exist_ok=True)
...
def upload_file_to_nextcloud(file_path: Path, remote_filename: str, content_type: str):
    if not nextcloud_configured():
        return

    ensure_nextcloud_folder()

    remote_path = f"{NEXTCLOUD_REMOTE_DIR}/{remote_filename}"
    url = build_webdav_url(remote_path)
...
def upload_excel_file_to_nextcloud(file_path: Path):
    upload_file_to_nextcloud(
        file_path=file_path,
        remote_filename=NEXTCLOUD_REMOTE_FILE,
        content_type=(
...
def upload_text_file_to_nextcloud(file_path: Path):
    remote_txt_filename = Path(NEXTCLOUD_REMOTE_FILE).with_suffix(".txt").name
    upload_file_to_nextcloud(
        file_path=file_path,
...
def append_and_sync(entry: dict):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir_name:
        ...
        if nextcloud_configured() and not is_debug_mode():
        ...
        export_entries_to_excel(LOCAL_EXCEL_EXPORT_FILE)
        export_entries_to_text(LOCAL_TEXT_EXPORT_FILE)

        if nextcloud_configured():
            upload_excel_file_to_nextcloud(LOCAL_EXCEL_EXPORT_FILE)
            upload_text_file_to_nextcloud(LOCAL_TEXT_EXPORT_FILE)
